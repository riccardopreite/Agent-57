...
def create_parameter(env, nb_steps):
    ...
    if image:
        ...
        input_model =  ValueModel(32, 1)#DQNImageModel()
    else:
    ...
    kwargs = {
        ...
        "optimizer_ext": Adam(learning_rate=0.0005),
        "optimizer_int": Adam(learning_rate=0.0005),
        "optimizer_rnd": Adam(learning_rate=0.001),
        "optimizer_emb": Adam(learning_rate=0.001),

        # NN
        "batch_size": 4,#16,     # batch_size
...
class MyActor(ActorUser):
    ...
    def fit(self, index, agent):
        ...
        # agent.train_on_batch(env, visualize=False, verbose=0)
        env.close()
